...
train1=dfd[~dfd['condition'].isna()]
train1=train1.set_index('pet_id')
train1['color_type']=train1['color_type'].apply(xx)
aa=pd.get_dummies(train1['color_type'].apply(pd.Series).stack(), 
                        columns=['color_type']).sum(level=0)
train1=pd.concat([train1, aa], axis=1, sort=False)


train1_x=train1.iloc[:,2:]
train1_y=train1.loc[:,['condition']]


pred_x=dfd[dfd['condition'].isna()]
pred_x=pred_x.set_index('pet_id')
pred_x=pred_x.drop('condition', axis=1)
pred_x['color_type'] = pred_x['color_type'].apply(xx)
bb=pd.get_dummies(pred_x['color_type'].apply(pd.Series).stack(),
                   columns=['color_type']).sum(level=0)
pred_x = pd.concat([pred_x, bb], axis=1, sort=False)
pred_x = pred_x.drop(['color_type'], axis=1)
...
for x in a1:
    train1_x[x]=[0]*len(train1_x.index)
for x in b1:
...
knn_cl_condition1.fit(train1_x, train1_y)
